...
void
flipit(GLint flip, int flipped, float xrot)
{
	Mat4 flipmatrix;

	if(flipped)
		xm4_rotate_x(xm(flipmatrix), xrot);
	else
		xm4_rotate_x(xm(flipmatrix), 180-xrot);
	glUniformMatrix4fv(flip, 1, GL_TRUE, xm(flipmatrix));
}

int
uiloop(GLint model, GLint flip)
{
	SDL_Event e;
	SDL_Keycode keysym;
